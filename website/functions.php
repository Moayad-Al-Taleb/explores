...
function photos_ActivityID($ActivityID, $number)
{
    require 'connect.php';
    try {
        $query = "SELECT PhotoID, picture FROM photos WHERE ActivityID = :ActivityID  LIMIT 1";
        ...
        if ($number == 1) {
            ...
            $query = "SELECT picture FROM photos WHERE ActivityID = :ActivityID AND PhotoID != :PhotoID";
            $stmt = $conn->prepare($query);
            $stmt->execute(
                [
                    ':ActivityID' => $ActivityID,
